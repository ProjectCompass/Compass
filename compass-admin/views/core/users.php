...
switch ($screen) {
    case 'submenu':
        echo 
            ...
            make_menu('submenu_item', lang('users_menu_insert'), 'users', 'insert', 'perm_insertusers_').
            make_menu('submenu_item', lang('users_menu_settings'), 'users', 'settings', 'perm_userssettings_').
            make_menu('submenu_item', lang('users_menu_permissions'), 'users', 'permissions', 'perm_userspermissions_', get_setting('users_advanced_settings')).
    ...
    case 'users':
        echo '<div id="users-index" class="row">';
            echo '<div class="small-8 columns">';
                ...
                echo anchor('users/insert', lang('users_menu_insert'), 'class="addimg button button-tiny secondary radius space-v-medium space-h-small"');
            echo '</div>';
                    ...
                    if ($line->user_status != 9):
                        ...
                        echo '<tr>';
                            ...
                            printf("<strong>%s</strong><br>%s %s</td>",
                                ...
                                (access('perm_updateusers_', 'high', TRUE, $this->users->get_by_id($line->user_id)->row()->user_level, $this->users->get_by_id($line->user_id)->row()->user_username)) ? anchor("users/update/$line->user_id", lang('core_edit'), array('class'=>'table-action table-action-first update', 'title'=>lang('core_edit'))) : NULL, 
                ...
                echo form_open('users/insert', 'id="users-insert-form-insert"');
                    ...
                    echo '<div class=row>';
                        echo '<div class="small-12 columns">';
                            echo form_submit(array('name'=>'save', 'class'=>'button radius small tiny'), lang('users_insert'));
                        echo '</div>';
                    echo '</div>';
                echo form_close();
            echo '</div>';
        ...
        echo '<div id="users-update" class="row">';
            echo '<div class="small-12 medium-10 large-8 columns">';
                ...
                echo form_open_multipart(current_url(), 'id="users-update-form-update"');
                    ...
                    echo '<div class=row><div class="small-3 columns">'.form_submit(array('name'=>'save', 'class'=>'button radius small tiny'), lang('users_save')).'</div></div>';
                echo form_close();
            echo '</div>';
        echo '</div>';
        break;
            ...
            echo '<div class="small-12 columns">';
                echo form_open(current_url(), 'id="users-settings-form-general"');
                    ...
                    echo '<div class="row">';
                        ...
                        echo '</div>';
                    echo '</div>';
                    echo form_submit(array('name'=>'save', 'class'=>'button radius tiny'), lang('users_settings_save'));
                ...
                echo '<div class="row">';
                    ...
                    echo '<div class="small-5 columns">';
                        ...
                        if ($this->uri->segment(3) == NULL):
                            set_theme('headerinc', load_module('includes', 'elementshiden'), FALSE);
                        endif;
                                    ...
                                    foreach ($query as $line):
                                        ...
                                        echo '<tr>';
                                            ...
                                            printf('<td class="table-operations"><strong>%s</strong><br>%s %s</td>', $line->term_name,
                                                anchor("users/settings/updateuserfield/$line->term_id", lang('core_edit'), array('class'=>'table-actions update', 'title'=>lang('core_edit'))), 
                                ...
                                foreach ($query as $line):   
                                    echo '<tr>';
                                        ...
                                        printf('<td class="table-operations"><strong>%s</strong><br>%s %s</td>', $line->userlevel_name,
                                            anchor("users/settings/updateuserlevel/$line->userlevel_id", lang('core_edit'), array('class'=>'table-actions update', 'title'=>lang('core_edit'))), 
        ...
        echo '<div id="users-permissions" class="row">';
            echo '<div class="small-12 columns">';
                echo form_open(current_url(), 'id="users-settings-form-permissions"');
                    echo '<h3>'.lang('users_permissions').'</h3>';
                    echo '<table id="users-settings-table-permissions">';
                        echo '<thead>';
                            echo '<tr>';
                                echo '<th class="small-2 collums">'.lang('users_permissions_userlevel').'</th>';
                                    $query_userslevels = $this->userslevels->get_all()->result();
                                    foreach ($query_userslevels as $line):
                        ...
                        echo '<tbody>';
                            $alllabelspermissions = array(
                                lang('users_permissions_users_active'),
                                    lang('core_cms'),
                                ...
                                'Alterar configurações de livros',
                                    lang('core_users'),
                                lang('users_permissions_list_users'),
                                lang('users_permissions_view_profiles'),
                                lang('users_permissions_insert_users'),
                                lang('users_permissions_update_users'),
                                lang('users_permissions_update_levels'),
                                lang('users_permissions_update_status'),
                                lang('users_permissions_users_delete'),
                                lang('users_permissions_users_settings'),
                                lang('users_permissions_update_perm'),
                                    lang('core_settings'),
                                lang('users_permissions_settings'),
                                    lang('core_tools'),
                                lang('users_permissions_tools'),
                                );
                        ...
                        echo '</tbody>';
                    echo '</table>';
                    echo form_submit(array('name'=>'save', 'class'=>'button radius tiny'), 'Salvar Configuração');
                echo form_close();
            echo '</div>';
        echo '</div>';
        break;
