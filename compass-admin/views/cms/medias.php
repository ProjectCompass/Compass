...
switch ($screen) {
    ...
    case 'uploadmediasmodal':
        echo '<div id="modalimg-upload" class="reveal-modal small" data-reveal >';
            echo '<div class="row">';
                echo '<iframe src="'.base_url().'cms/uploadmediasmodal/" class="small-12 columns" height="250px" scrolling="no" frameborder="0"></iframe>';
            echo '</div>';
            echo '<div class="row collapse">';
                    ...
                    echo '<div class="small-4 columns">';
                        ...
                        echo form_open_multipart(current_url(), 'id="cms-medias-form-insert"');
                            ...
                            if ($actionmedia != 'saved'): 
                                ...
                                if ($idmedia != NULL && $actionmedia == 'update'):
                                    echo anchor('cms/medias', ' '.lang('cms_cancel'), array('class'=>'alertlink'));
                                endif;
                            else:
                                $query_media = $this->posts->get_by_id($this->uri->segment(4))->row();
                                echo '<div class="row">';
                                    echo '<div class="small-12 columns">';
                                        echo thumb($query_media->post_content, 300, 180);
                                    echo '</div>';
                                    echo '<div class="small-12 columns">';
                                        echo '<div class="row collapse">';
                                            ...
                                            echo '</div>';
                                            echo '<div class="small-2 columns">';
                                                echo anchor_popup(base_url('compass-content/uploads/medias').'/'.$query_media->post_content, '<i class="fa fa-picture-o"></i>', array('class'=>'button tiny', 'title'=>lang('cms_visualize')));
                                            echo '</div>';
                                        echo '</div>';
                                    echo '</div>';
                                    echo '<div class="small-12 columns">';
                                    ...
                                    if ($line->post_type == 'media'):
                                        echo '<tr>';
                                            ...
                                            printf('<td class="table-operations"><strong>%s</strong><br>%s %s</td>', 
                                                anchor_popup(base_url('compass-content/uploads/medias').'/'.$line->post_content, $line->post_title, array('title'=>lang('cms_visualize'))),
                                                anchor("cms/medias/update/$line->post_id", lang('cms_update'), array('class'=>'table-action table-action-first update', 'title'=>lang('cms_update'))), 
                                                anchor("cms/medias/delete/$line->post_id", lang('cms_delete'), array('class'=>'table-action delete deletereg', 'title'=>lang('cms_delete'))));
