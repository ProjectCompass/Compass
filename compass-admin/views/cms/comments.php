    ...
    case 'comments':
        echo '<div class="row" id="cms-comments">';
            echo '<div class="small-12 columns">';
                ...
                echo '</div>';
                echo '<div class="row">';
                    $show_updatecomment = ($this->uri->segment(3) != 'update' && $this->uri->segment(3) != 'reply') ? 'hide' : NULL;
                    $show_largecomments = ($this->uri->segment(3) != 'update' && $this->uri->segment(3) != 'reply') ? 'small-12' : 'small-8';
                    echo '<div class="small-4 '.$show_updatecomment.' columns">';
                        $query_comment = NULL;
                        if ($this->uri->segment(3) == 'update' && $this->uri->segment(4) != NULL):
                            $query_comment = $this->comments->get_by_id($this->uri->segment(4))->row();
                        endif;
                        echo '<h5>';
                            echo ($this->uri->segment(3) == 'update') ? lang('cms_comments_update') : lang('cms_comments_reply');
                        echo '</h5>';
                        echo form_open(current_url(), 'id="cms-comments-form-update"');
                            ...
                            echo '<div class="row">';
                                echo '<div class="small-12 columns">';
                                    ...
                                    echo form_input(array('name'=>'comment_author', 'placeholder'=>lang('cms_comments_field_name')), set_value('comment_author', ($this->uri->segment(3) == 'update') ? $query_comment->comment_author : get_session('user_displayname')), 'autofocus');
                                    echo form_label(lang('cms_comments_field_email'));
                                    echo form_input(array('name'=>'comment_authoremail', 'placeholder'=>lang('cms_comments_field_email')), set_value('comment_authoremail', ($this->uri->segment(3) == 'update') ? $query_comment->comment_authoremail : get_session('user_email')));
                                    echo form_label(lang('cms_comments_field_url'));
                                    echo form_input(array('name'=>'comment_authorurl', 'placeholder'=>lang('cms_comments_field_url')), set_value('comment_authorurl', ($this->uri->segment(3) == 'update') ? $query_comment->comment_authorurl : get_usermeta('user_url', get_session('user_id'))));
                                    echo form_label(lang('cms_comments_field_comment'));
                                    echo form_textarea(array('name'=>'comment_content', 'rows'=>'5', 'placeholder'=>lang('cms_comments_field_comment')), set_value('comment_content', ($this->uri->segment(3) == 'update') ? $query_comment->comment_content : NULL));
                                    echo form_label(lang('cms_comments_field_date'));
                                    echo form_input(array('name'=>'comment_date', 'class'=>'datetimepicker'), set_value('comment_date', ($this->uri->segment(3) == 'update') ? get_gmt_to_local_date($query_comment->comment_date) : get_gmt_to_local_date(date('Y-m-d H:i:s'))));
                                echo '</div>';
                            echo '</div>';
                            echo form_hidden('idcomment', $this->uri->segment(4));
                            echo form_hidden('idpost', isset($query_comment->comment_postid));
                            echo form_submit(array('name'=>($this->uri->segment(3) == 'update') ? 'update' : NULL, 'class'=>'button radius tiny'), ($this->uri->segment(3) == 'update') ? lang('cms_comments_update') : NULL);
                            echo anchor('cms/comments', ' '.lang('cms_cancel'), array('class'=>'alertlink'));
                        echo form_close();
                    echo '</div>';
                                ...
                                foreach (get_query($config)->result() as $line):
                                    ...
                                    echo '<tr>';
                                        printf('<td class="table-operations">'.lang('cms_comments_posted_in').': %s<br>%s '.lang('cms_comments_in_post').': %s<br>%s%s%s%s</td>',
                                            ...
                                            anchor("site/post/$line->comment_postid", $query_post->post_title, array('target'=>'_black')),
                                            ($line->comment_status == 'approved' || $line->comment_status == 'unmoderated' || $line->comment_status == 'trash') ? anchor("cms/comments/unapprove/$line->comment_id", lang('cms_comments_action_reject'), array('class'=>'table-action table-action-first unapprove', 'title'=>lang('cms_comments_action_reject_t'))) : NULL,
                                            ($line->comment_status == 'unapproved' || $line->comment_status == 'unmoderated' || $line->comment_status == 'trash') ? anchor("cms/comments/approve/$line->comment_id", lang('cms_comments_action_approve'), array('class'=>'table-action table-action-first approve', 'title'=>lang('cms_comments_action_approve_t'))) : NULL,
                                            anchor("cms/comments/update/$line->comment_id", lang('cms_comments_action_update'), array('class'=>'table-action update', 'title'=>lang('cms_comments_action_update_t'))),
                                            ($line->comment_status != 'trash') ? anchor("cms/comments/delete/$line->comment_id", lang('cms_comments_action_delete'), array('class'=>'table-action delete deletereg', 'title'=>lang('cms_comments_action_delete_t'))) : NULL);
