                    ...
                    echo '<div class="small-3 columns">';
                        ...
                        echo '<ul id="drop1" data-dropdown-content class="f-dropdown">';                            
                            foreach ($all_tags as $line):
                                echo '<li><a href="'.base_url('cms/posts/0/orderby/post_modified/order/desc/filter_tag').'/'.str_replace(' ', '-', $line).'">'.ucfirst($line).'</a></li>';
                            endforeach;
                        echo '</ul>';
                    echo '</div>';
                            ...
                            if ($line->post_type == 'post'):
                                echo '<tr>';
                                    ...
                                    printf("<td class='table-operations'><strong>%s</strong><br>%s %s %s</td>",
                                        ...
                                        (access('perm_updateposts_', NULL, TRUE)) ? anchor("cms/posts/update/$line->post_id", lang('cms_update'), array('class'=>'table-action table-action-first update', 'title'=>lang('cms_update'))) : NULL,
                                        (access('perm_viewposts_', NULL, TRUE)) ? anchor("site/post/$line->post_slug", lang('cms_visualize'), array('class'=>'table-action view', 'title'=>lang('cms_visualize').' \''.$line->post_title.'\'', 'target'=>'_blanck')) : NULL,
                                        (access('perm_deleteposts_', NULL, TRUE)) ? anchor("cms/posts/delete/$line->post_id", lang('cms_delete'), array('class'=>'table-action delete deletereg', 'title'=>lang('cms_delete'))) : NULL
                                        );
                                    ...
                                    if ($line->post_tags != NULL):
                                        ...
                                        foreach ($query_tags as $line_tag):
                                            $tags[] = anchor("cms/posts/0/orderby/post_modified/order/desc/filter_tag/".str_replace(' ', '-', $line_tag), ucfirst($line_tag));
                                        endforeach;
                                    endif;
                            ...
                            echo '<div class="row collapse">';
                                ...
                                echo '<div class="small-4 columns">';
                                    echo '<div class="row collapse">';
                                        ...
                                        echo anchor('site/post/'.$query->post_slug, lang('cms_visualize'), array('class'=>'small-3 button tiny secondary right', 'title'=>lang('cms_visualize'), 'target'=>'_blanck'));
                                    echo '</div>';
                                echo '</div>';
                            echo '</div>';
                        echo '</div>';
