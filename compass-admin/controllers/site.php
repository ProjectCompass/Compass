...
class Site extends CI_Controller {
	...
	public function __construct(){
		parent::__construct();
		//loads utilities
		$this->load->helper(array('cms', 'url'));
		$this->load->model('users_model', 'users');
		$this->load->model('comments_model', 'comments');
		$this->load->model('posts_model', 'posts');
		//Blocks access to the site if the cms module is disabled
		if (get_setting('module_cms') != 1) redirect(base_url());
	}
	...
	public function post(){
		...
		$byid_or_byslug = (is_numeric($this->uri->segment(3))) ? 'get_by_id' : 'get_by_slug';
		//if no value is passed id or slug in the url redirects to the main stop
		if ($this->uri->segment(3) == NULL || $this->posts->$byid_or_byslug($this->uri->segment(3))->row()->post_id == NULL) redirect(base_url('site/error'));
		//Arrow visitation statistics
		set_stat();
		//updates the number of comments for this post
		$data_posts['post_comment_count'] = $this->comments->get_by_postid($this->posts->$byid_or_byslug($this->uri->segment(3))->row()->post_id)->num_rows();
		$this->posts->do_update($data_posts, array('post_id'=>$this->posts->$byid_or_byslug($this->uri->segment(3))->row()->post_id), FALSE, FALSE);
	...
	public function media(){
		//if no value is passed id or slug in the url redirects to the main stop
		if ($this->uri->segment(3) == NULL) redirect(base_url('site/error'));
		//arrow visitation statistics
		set_stat();
