...
function get_menu($config=NULL, $num_max=25){
	...
	foreach ($query as $line):
		if ($count < $num_max):
			echo $menu_item_open.'<a href="'.base_url('site/page/').'/'.$line->post_slug.'">'.$line->post_title.'</a>'.$menu_item_close;
		endif;
		$count ++;
	endforeach;
	echo $menu_close;
...
function query_posts($config=array(), $post_type='post'){
	...
	if ($CI->uri->segment(2) == 'post'):
		$CI->db->where('post_id', $CI->uri->segment(3));
		if (is_numeric($CI->uri->segment(3))):
			$CI->db->or_where('post_id', $CI->uri->segment(3));
		else:
			$CI->db->or_where('post_slug', $CI->uri->segment(3));
		endif;
		$CI->db->limit(1);
		...
		if ($CI->uri->segment(2) == 'author'):
			$CI->db->where('post_author', $CI->uri->segment(3));
		...
		if ($CI->uri->segment(2) == 'search'):
			$CI->db->like('post_title', str_replace('-', ' ', $CI->uri->segment(3)));
            $CI->db->or_like('post_date', str_replace('-', ' ', $CI->uri->segment(3)));
            $CI->db->or_like('post_excerpt', str_replace('-', ' ', $CI->uri->segment(3)));
            $CI->db->or_like('post_content', str_replace('-', ' ', $CI->uri->segment(3)));
		endif;
		if ($tags != NULL):
			$tags = explode(',', $tags);
			foreach ($tags as $tag):
...
function pagination($config=NULL, $post_type='post'){
	...
	foreach ($config as $k => $v) eval("\$".$k." = \"".$v."\";");
	$pagination_segment_value = ($CI->uri->segment($pagination_segment)) ? $CI->uri->segment($pagination_segment) : 1;
	$pagination_url = base_url().'site/'.$pagination_url.'p/';
	...
	if($css == TRUE && get_setting('site_menu_tools') == 1):
		...
		echo '<ul class="left-bar">';
		    ...
		    echo '<li><a title="Novo"><img src="'.base_url().'/compass-content/includes/images/new.png" /></a>';
		      	echo '<ul>';
			        echo '<li><a href="'.base_url('cms/insertpost').'" title="Novo post">Post</a></li>';
			        echo '<li><a href="'.base_url('cms/insertpage').'" title="Nova página">Página</a></li>';
		    ...
		    if ($CI->uri->segment(3) == 'post'):
		    	echo '<li><a href="'.base_url('contents/updatepost/'.$CI->uri->segment(3)).'"><img src="'.base_url().'/compass-content/includes/images/edit.png" /></a></li>';
		    elseif ($CI->uri->segment(3) == 'page'):
		    	echo '<li><a href="'.base_url('contents/updatepage/'.$CI->uri->segment(3)).'"><img src="'.base_url().'/compass-content/includes/images/edit.png" /></a></li>';
		    endif;
		echo '</ul>';
	endif;
}
...
function the_permalink($query=NULL){
	...
	$byid_or_byslug = (is_numeric($CI->uri->segment(3))) ? 'get_by_id' : 'get_by_slug';
	if ($CI->uri->segment(2) == 'post'):
		echo base_url('site/post').'/'.$CI->posts->$byid_or_byslug($CI->uri->segment(3))->row()->post_slug;
	elseif ($CI->uri->segment(2) == 'page'):
		echo base_url('site/page').'/'.$CI->posts->$byid_or_byslug($CI->uri->segment(3))->row()->post_slug;
	elseif ($CI->uri->segment(2) == 'media'):
		echo base_url('site/media').'/'.$CI->posts->$byid_or_byslug($CI->uri->segment(3))->row()->post_slug;
	else:
		echo base_url('site/post').'/'.$query->post_slug;
	endif;
}
...
function the_title($query=NULL){
	...
	$byid_or_byslug = (is_numeric($CI->uri->segment(3))) ? 'get_by_id' : 'get_by_slug';
	if ($CI->uri->segment(2) == 'post' || $CI->uri->segment(2) == 'page' || $CI->uri->segment(2) == 'media'):
		echo $CI->posts->$byid_or_byslug($CI->uri->segment(3))->row()->post_title;
	else:
...
function the_author($query=NULL, $islink=FALSE){
	...
	$byid_or_byslug = (is_numeric($CI->uri->segment(3))) ? 'get_by_id' : 'get_by_slug';
	if ($CI->uri->segment(2) == 'post' || $CI->uri->segment(2) == 'page' || $CI->uri->segment(2) == 'media'):
		$user = $CI->users->get_by_id($CI->posts->$byid_or_byslug($CI->uri->segment(3))->row()->post_author)->row();
	else:
...
function the_author_image($query=NULL, $isimg=FALSE){
	...
	$byid_or_byslug = (is_numeric($CI->uri->segment(3))) ? 'get_by_id' : 'get_by_slug';
	if ($CI->uri->segment(2) == 'post' || $CI->uri->segment(2) == 'page' || $CI->uri->segment(2) == 'media'):
		$user_screenshot = get_usermeta('user_image', $CI->posts->$byid_or_byslug($CI->uri->segment(3))->row()->post_title);
	else:
...
function the_date($query=NULL, $format=NULL){
	...
	if ($CI->uri->segment(2) == 'post' || $CI->uri->segment(2) == 'page' || $CI->uri->segment(2) == 'media'):
		if ($format != NULL):
			echo date($format, strtotime($CI->posts->$byid_or_byslug($CI->uri->segment(3))->row()->post_date_publish));
		else:
			if (get_setting('system_date_format') || get_setting('system_time_format')):
				echo date(get_setting('system_date_format'), strtotime($CI->posts->$byid_or_byslug($CI->uri->segment(3))->row()->post_date_publish));
			else:
				echo $CI->posts->$byid_or_byslug($CI->uri->segment(3))->row()->post_date_publish;
			endif;
		endif;
	else:
		if ($format != NULL):
...
function the_time($query=NULL, $format=NULL){
	...
	if ($CI->uri->segment(2) == 'post' || $CI->uri->segment(2) == 'page' || $CI->uri->segment(2) == 'media'):
		if ($format != NULL):
			...
			if (get_setting('system_date_format') || get_setting('system_time_format')):
				echo date(get_setting('system_date_format').' '.get_setting('system_time_format'), strtotime($CI->posts->$byid_or_byslug($CI->uri->segment(3))->row()->post_date_publish));
			else:
				echo $CI->posts->$byid_or_byslug($CI->uri->segment(3))->row()->post_date_publish;
			endif;
		endif;
...
function the_content($query=NULL){
	...
	$CI->load->model('posts_model', 'posts');
	$byid_or_byslug = (is_numeric($CI->uri->segment(3))) ? 'get_by_id' : 'get_by_slug';
	if ($CI->uri->segment(2) == 'post' || $CI->uri->segment(2) == 'page'):
		echo $CI->posts->$byid_or_byslug($CI->uri->segment(3))->row()->post_content;
	elseif ($CI->uri->segment(2) == 'media'):
		if (substr($CI->posts->$byid_or_byslug($CI->uri->segment(3))->row()->post_content, -3) == 'gif' || substr($CI->posts->$byid_or_byslug($CI->uri->segment(3))->row()->post_content, -3) == 'jpg' || substr($CI->posts->$byid_or_byslug($CI->uri->segment(3))->row()->post_content, -3) == 'png'):
			echo '<img src="'.base_url().'/compass-content/uploads/medias/'.$CI->posts->$byid_or_byslug($CI->uri->segment(3))->row()->post_content.'" />';
		else:
			echo '<a href="'.base_url().'/compass-content/uploads/medias/'.$CI->posts->$byid_or_byslug($CI->uri->segment(3))->row()->post_content.'">'.base_url().'/compass-content/uploads/medias/'.$CI->posts->$byid_or_byslug($CI->uri->segment(3))->row()->post_content.'</a>';
		endif;
	else:
...
function the_excerpt($query=NULL, $chars=NULL){
	...
	$byid_or_byslug = (is_numeric($CI->uri->segment(3))) ? 'get_by_id' : 'get_by_slug';
	if ($CI->uri->segment(2) == 'post' || $CI->uri->segment(2) == 'page' || $CI->uri->segment(2) == 'media'):
		echo $CI->posts->$byid_or_byslug($CI->uri->segment(3))->row()->post_excerpt;
	else:
...
function the_comments_count($query=NULL){
	...
	$byid_or_byslug = (is_numeric($CI->uri->segment(3))) ? 'get_by_id' : 'get_by_slug';
	if ($CI->uri->segment(2) == 'post' || $CI->uri->segment(2) == 'page' || $CI->uri->segment(2) == 'media'):
		echo $CI->posts->$byid_or_byslug($CI->uri->segment(3))->row()->post_comment_count;
	else:
...
function the_views($query=NULL){
	...
	$byid_or_byslug = (is_numeric($CI->uri->segment(3))) ? 'get_by_id' : 'get_by_slug';
	if ($CI->uri->segment(2) == 'post' || $CI->uri->segment(2) == 'page' || $CI->uri->segment(2) == 'media'):
		echo $CI->posts->$byid_or_byslug($CI->uri->segment(3))->row()->post_views;
	else:
...
function the_category($query=NULL, $separator=', ', $islink=FALSE){
	...
	$byid_or_byslug = (is_numeric($CI->uri->segment(3))) ? 'get_by_id' : 'get_by_slug';
	if ($CI->uri->segment(2) == 'post' || $CI->uri->segment(2) == 'page' || $CI->uri->segment(2) == 'media'):
		$post_id = $CI->posts->$byid_or_byslug($CI->uri->segment(3))->row()->post_id;
...
function the_screenshot($query=NULL, $isimg=FALSE, $width=300, $height=300){
	...
	$byid_or_byslug = (is_numeric($CI->uri->segment(3))) ? 'get_by_id' : 'get_by_slug';
	if ($CI->uri->segment(2) == 'post' || $CI->uri->segment(2) == 'page' || $CI->uri->segment(2) == 'media'):
		$post_screenshot = get_postmeta('image', $CI->posts->$byid_or_byslug($CI->uri->segment(3))->row()->post_id);
	else:
...
function the_comments($title='Comments', $config=array()){
	...
	if ($CI->posts->$byid_or_byslug($CI->uri->segment(3))->row()->post_comment_count > 0):
		$query_comments = $CI->comments->get_by_postid($CI->posts->$byid_or_byslug($CI->uri->segment(3))->row()->post_id)->result();
...
function the_comment_form($title='Write your own review', $attributes=NULL){
	...
	$CI->load->model('posts_model', 'posts');
	$byid_or_byslug = (is_numeric($CI->uri->segment(3))) ? 'get_by_id' : 'get_by_slug';
	if ($CI->posts->$byid_or_byslug($CI->uri->segment(3))->row()->post_comment_status == 1):
		if ($title != NULL):
			echo '<h5>'.$title.'</h5>';
		endif;
		echo form_open(current_url(), ($attributes == NULL) ? 'id="comment_form"' : $attributes);
	        ...
	        echo form_hidden('idpost', $CI->posts->$byid_or_byslug($CI->uri->segment(3))->row()->post_id);
	...
	foreach (query_posts($query_conFig, 'post') as $post):
		echo '<div class="row collapse '.$attributes.'">';
			...
			echo '<div class="small-9 columns">';
				echo '<h6><a href="'.base_url('site/post/'.$post->post_slug).'">'.$post->post_title.'</a></h6>';
			echo '</div>';
		echo '</div>';
	endforeach;
}
...
function the_tags($attributes=NULL){
    ...
    foreach ($all_tags as $line):
    	echo anchor(base_url('site/tag/'.str_replace(' ', '-', $line)), ucfirst($line), 'class="button tiny button-category '.$attributes.'"');
    endforeach;
}
...
function the_search_form($attributes=NULL){
	...
	echo form_open(base_url('site/search', $attributes));
